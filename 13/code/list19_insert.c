...
void insert_servant(SERVANT **now, char *class, char *np);
...
int main(){
    ...
    p = &lancer;
    insert_servant(&p,"AUO\0","王の財宝\0"); //lancerの次にノードを代入
...
void insert_servant(SERVANT **now, char *class, char *np){
    //next ポインタを新しく動的確保により生成する 
    SERVANT *p;
    p=(SERVANT*)malloc(sizeof(SERVANT));

    //値を代入する
    strcpy(p->class, class);
    strcpy(p->np, np);
    
    //ポインタをすり替えて新しいデータを挿入する
    p->next = ((*now)->next!=NULL)?(*now)->next:NULL;
    (*now)->next = p;
