...
*具体的にはまだ習っていない繰り返し文,条件分岐,配列の知識が必要不可欠です。\\
\\
榊くんの挑戦状EX(未履修範囲の知識が必要な問題)\\
1万個の要素に対して最大値、最小値、平均値、最頻値を求めるプログラムを作成できた人には榊くんが晩御飯を奢ります。\\
要素の挿入にはrand()関数を用いて要素を挿入すること(N=10000で10000個手入力するのは頭が壊れるのだ)。\\
なお、この問題の模範解法は配布しない。講座終了後、知りたければその場でコーディングするので榊まで聞きに来てください。\\
(晩御飯奢り権は新学部1年生先着1名限定)\\
(平均値まではインターネットで調べてコードのコピペが容易なので、最頻値も求めさせます)\\
\end{document}
